...
class TankSimulation:
    ...
    def _pre_render_liquid_shape(self):
        ...
        self.tank_points = [p1, p2, p3, p6, p7, p8, p5, p4]
    ...
    def draw(self):
        ...
        if self.current_level > 0:
            ...
            clear_rect = pygame.Rect(0, 0, WIDTH, int(liquid_top_y))
        ...
        if self.is_filling and self.current_level < self.total_height:
            pygame.draw.line(self.screen, BLUE, (self.center_x, self.start_y - 50), 
                             (self.center_x, self.start_y + (self.total_height - self.current_level)), 8)
            
        if self.is_draining and self.current_level > 0:
            pygame.draw.line(self.screen, BLUE, (self.center_x, self.start_y + self.total_height), 
                             (self.center_x, HEIGHT), 6)
    ...
    def draw_ui(self):
        ...
        status_in = "OTWARTY" if self.is_filling else "ZAMKNIĘTY"
        status_out = "OTWARTY" if self.is_draining else "ZAMKNIĘTY"
        self.screen.blit(font.render(f"Wlot: {status_in} (Strzałka w górę)", True, WHITE), (10, 35))
        self.screen.blit(font.render(f"Wylot: {status_out} (Strzałka w dół)", True, WHITE), (10, 60))
